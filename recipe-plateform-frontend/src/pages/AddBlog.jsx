...
//   const [imageToCrop, setImageToCrop] = useState(null); // dataURL
//   const [crop, setCrop] = useState({ x: 0, y: 0 });
//   const [zoom, setZoom] = useState(1);
//   const [croppedAreaPixels, setCroppedAreaPixels] = useState(null);
//   const [aspect, setAspect] = useState(4 / 3);

//   // image-editing/selection states
//   const [selectedImg, setSelectedImg] = useState(null); // DOM element
//   const [imgControlPos, setImgControlPos] = useState({ top: 0, left: 0, visible: false });
//   const [imgWidthPct, setImgWidthPct] = useState(100); // percentage of editor width
//   const [imgWidthPx, setImgWidthPx] = useState(null); // px width
//   const [imgNaturalWidth, setImgNaturalWidth] = useState(null);
//   const [imgAlign, setImgAlign] = useState("center"); // left, center, right
...
//   // helper to run document commands
...
//   // when user selects a file -> open inline cropper
...
//   // utility: create cropped image dataURL from source + pixel area
...
//   // confirm cropping: insert into editor and save featured image
...
//     // insert into editor at caret location
//     if (editorRef.current) {
//       editorRef.current.focus();
//       const imgHTML =
//         `<img class="inserted-image" src="${croppedDataUrl}" ` +
...
//   // ---------- Image selection & controls logic ----------

//   // Helper: compute panel position near image (within editor wrapper bounds)
//   const computeControlPosition = (imgEl) => {
//     const editorRect = editorRef.current.getBoundingClientRect();
//     const imgRect = imgEl.getBoundingClientRect();
//     // place panel above image if enough space, else below
...
//     // align left edge of panel with image left (clamped)
//     let left = imgRect.left - editorRect.left;
//     // clamp to editor width
//     const editorWidth = editorRect.width;
//     if (left + 320 > editorWidth) {
//       left = Math.max(8, editorWidth - 320 - 8);
//     }
//     return { top: Math.round(top), left: Math.round(left) };
//   };

//   // Apply sizing/align to selected image DOM element
//   const applyImageStyle = (imgEl, { pct = null, px = null, align = "center" } = {}) => {
//     if (!imgEl) return;
//     // remove float/align first
//     imgEl.style.float = "";
//     imgEl.style.display = "";
...
//       imgEl.style.maxWidth = "none"; // allow px width even if > container
//     } else if (pct) {
//       // set width as percent of editor content width
...
//     // always keep height auto to preserve aspect
//     imgEl.style.height = "auto";
//   };

//   // select an image (from click)
//   const selectImage = (imgEl) => {
//     if (!imgEl) return;
//     // store selected element
//     setSelectedImg(imgEl);
//     // compute natural width & current width
//     const natural = imgEl.naturalWidth || imgEl.width || null;
//     setImgNaturalWidth(natural);

//     // compute percentage relative to editor width if applicable
//     const editorWidth = editorRef.current ? editorRef.current.clientWidth : null;
//     let pct = 100;
//     if (editorWidth && imgEl.clientWidth) {
//       pct = Math.round((imgEl.clientWidth / editorWidth) * 100);
//     }
//     setImgWidthPct(pct);
...
//   // when user clicks inside editor: if click on img -> select, else hide
//   useEffect(() => {
...
//         selectImage(el);
//       } else {
//         // hide controls if clicked outside image (but keep if clicked inside panel later)
//         setImgControlPos((p) => ({ ...p, visible: false }));
//         setSelectedImg(null);
//       }
//     };

//     // allow keyboard navigation to deselect (Escape)
//     const onKeyDown = (e) => {
//       if (e.key === "Escape") {
//         setImgControlPos((p) => ({ ...p, visible: false }));
//         setSelectedImg(null);
...
//   // reposition controls when window resizes or content changes
//   useEffect(() => {
//     const onResize = () => {
...
//     // update pixel input value to match
//     const editorWidth = editorRef.current ? editorRef.current.clientWidth : null;
...
//       setImgAlign("center");
//     } else {
//       // fallback: remove explicit width
...
//   // Remove selection if user navigates away or publishes
//   useEffect(() => {
//     return () => {
//       setSelectedImg(null);
...
//                 />
//                 <button style={styles.linkAddBtn} onClick={handleLinkInsert}>
//                   Add
//                 </button>
//               </div>
//             )}
...
//                 <button
//                   onClick={() => setAspect(1)}
//                   style={{ marginRight: 6, padding: "6px 8px", borderRadius: 6, border: "1px solid #ddd", background: aspect === 1 ? "#eee" : "#fff" }}
//                 >
//                   1:1
//                 </button>
//                 <button
//                   onClick={() => setAspect(4 / 3)}
//                   style={{ marginRight: 6, padding: "6px 8px", borderRadius: 6, border: "1px solid #ddd", background: aspect === 4 / 3 ? "#eee" : "#fff" }}
//                 >
//                   4:3
//                 </button>
//                 <button
//                   onClick={() => setAspect(16 / 9)}
//                   style={{ padding: "6px 8px", borderRadius: 6, border: "1px solid #ddd", background: aspect === 16 / 9 ? "#eee" : "#fff" }}
//                 >
//                   16:9
//                 </button>
//               </div>
...
//                   style={{ ...styles.cropBtn, ...styles.confirmBtn }}
//                 >
//                   Confirm
//                 </button>
//                 <button
...
//           <div
//             style={{
//               ...styles.imgControlPanel,
//               top: imgControlPos.top,
//               left: imgControlPos.left,
//             }}
//           >
//             <div style={{ fontWeight: 600 }}>Image controls</div>
...
//                 onClick={() => {
//                   // set px to natural width (or editor width)
...
//               <button
//                 style={{ ...styles.smallBtn, background: imgAlign === "left" ? "#efefef" : "#fafafa" }}
//                 onClick={() => setImgAlign("left")}
//               >
//                 Left
//               </button>
//               <button
//                 style={{ ...styles.smallBtn, background: imgAlign === "center" ? "#efefef" : "#fafafa" }}
//                 onClick={() => setImgAlign("center")}
//               >
//                 Center
//               </button>
//               <button
//                 style={{ ...styles.smallBtn, background: imgAlign === "right" ? "#efefef" : "#fafafa" }}
...
//               <button
//                 style={{ marginLeft: "auto", ...styles.applyBtn }}
//                 onClick={() => {
//                   // Apply current settings (ensures inline style applied and panel stays)
//                   if (selectedImg) {
...
/**
 * AddBlog.jsx
 * - All previous features preserved (cropper, emoji, link, headings)
 * - Image controls: resize %, px, presets, alignment
 * - New: Save (make current edit the baseline) & Revert (restore baseline) per-image.
 * - Changes only written to localStorage when Publish/Update clicked.
 */
